...
class Birthday(Field):
    def __init__(self, value):
        try:
            self.value = value 
            birthday = datetime.strptime(self.value,"%D-%m-%Y").date() # та перетворіть рядок на об'єкт datetime
            return birthday                                                        
        except ValueError:
...
class Record:
    ...
    def add_birthday(self, value):
        self.birthday = Birthday(value)            # функція додавання дня народження, створює об
